// Updated Navigation.js with better wallet connection UX
import React, { useState, useEffect } from 'react';
import { Link, useLocation } from 'react-router-dom';
import { FaWallet, FaList, FaShoppingCart, FaPlus, FaHome, FaEye } from 'react-icons/fa';
...
const Navigation = ({ web3Handler, account }) => {
  ...
  const NavLink = ({ item, isMobile = false }) => {
    ...
    const baseClasses = `flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-300 ${
    ...
    if (needsWallet) {
      return (
        ...
        >
          ...
          {!isMobile && <FaWallet className="text-xs opacity-60" />}
        </button>
      );
    }
    ...
    <header className={`sticky top-0 left-0 w-full z-50 py-4 px-4 transition-all duration-300 ${
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between p-1 bg-surface/60 backdrop-blur-md rounded-full border border-white/5 shadow-lg">
          ...
          {/* Mobile menu button */}
          <button 
            className={`lg:hidden relative z-50 w-8 h-6 flex flex-col justify-between mr-4 ${menuOpen ? 'open' : ''}`} 
            onClick={() => setMenuOpen(!menuOpen)}
            aria-label="Toggle navigation menu"
          >
            <span className={`h-0.5 bg-text rounded-sm w-full transition-all duration-300 ${menuOpen ? 'rotate-45 translate-y-[10px]' : ''}`}></span>
            <span className={`h-0.5 bg-text rounded-sm w-full transition-all duration-300 ${menuOpen ? 'opacity-0' : ''}`}></span>
            <span className={`h-0.5 bg-text rounded-sm w-full transition-all duration-300 ${menuOpen ? '-rotate-45 -translate-y-[10px]' : ''}`}></span>
          </button>
          ...
          <div className="pl-2 pr-1">
            {account ? (
              <div className="flex items-center gap-2">
                ...
                <div className="hidden md:flex items-center px-3 py-1 bg-green-500/10 border border-green-500/30 rounded-full">
        ...
        {/* Mobile Menu Overlay */}
        <div className={`lg:hidden fixed inset-0 bg-background/95 z-40 transition-all duration-300 backdrop-blur-lg ${
          ...
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <div className="mb-8">
              {account ? (
                <div className="text-center">
                  <div className="flex items-center justify-center px-4 py-2 bg-green-500/10 border border-green-500/30 rounded-full mb-2">
                    <div className="w-2 h-2 bg-green-500 rounded-full mr-2"></div>
                    <span className="text-green-600 text-sm font-medium">Wallet Connected</span>
                  </div>
                  <p className="text-text-secondary text-sm">{account.slice(0, 10)}...{account.slice(-8)}</p>
                </div>
              ) : (
                <div className="text-center">
                  <div className="flex items-center justify-center px-4 py-2 bg-yellow-500/10 border border-yellow-500/30 rounded-full mb-2">
            ...
            {!account && (
              <button 
                ...
                className="mt-8 flex items-center px-6 py-3 rounded-full text-white font-semibold bg-gradient-to-r from-primary to-purple-400"
              >
                <FaWallet className="mr-2" />
                Connect Wallet
              </button>
